...
$query="Select * from contactus where status='Unread'";
$result=mysqli_query($con,$query);
$nbr=mysqli_num_rows($result);

if($nbr==0){
    Echo "<h1> All the mail are Read</h1>";
}
else {
    ?>
...
<?php
        for($i=0; $i<$nbr;$i++)
        {
            ...
            echo "<tr>";
            echo "<td>$row[ConctatID]</td>";
            echo "<td>$row[FullName]</td>";
            echo "<td>$row[ContactEmail]</td>";
            echo "<td>$row[status]</td>";
            echo "<td><a href='markstatus.php?ConctatID=$row[ConctatID]'><img src='Mark.png' alt='edit' width='50px' height='50px'/></a></td>";
            echo "</tr>";
        }
         echo "</table>";
