...
<html lang="en">
  ...
  <div id="carouselExampleDark" class="carousel carousel-dark slide" data-bs-ride="carousel">
    ...
    $all = "select * from Pets natural join Users where Pets.status='Available'";
    $result = mysqli_query($con , $all);
    $nbr= mysqli_num_rows($result);
    for($i=0; $i<$nbr;$i++)
        {  $row= mysqli_fetch_assoc($result);
              echo"<div class=\"col-md-4 ftco-animate $row[petType]\">";
               echo"<div class=\"blog-entry align-self-stretch\">";
                  echo "<a href=\"adopt-single.php?PetID=$row[PetID]&pname=$row[PetName]&page=$row[Age]&psex=$row[Sex]&pcas=$row[Castrated]&pinfo=$row[MoreInfo]&ptype=$row[petType]&puser=$row[UserName]&img=$row[petImage]\" class=\"block-20 rounded\" style=\"background-image: url('petsImages/$row[petImage]');\">";
                  echo"</a>";
                  echo"<div class=\"text p-4\">";
                    echo"<div class=\"meta mb-2\">";
                     echo"<div>$row[PetName]</div>";
                      echo"<div>$row[UserName]</div>"; 
                     echo" <div>$row[petType]</div>";
                    echo"</div>";
                    echo"<h3 class=\"heading\"><a href=\"adopt-single.php?PetID=$row[PetID]&pname=$row[PetName]&page=$row[Age]&psex=$row[Sex]&pcas=$row[Castrated]&pinfo=$row[MoreInfo]&ptype=$row[petType]&puser=$row[UserName]&img=$row[petImage]\" >";
                    $s=explode(' ',$row["MoreInfo"]);
                    $new="";
                    if(count($s)<36){for($j=0; $j<count($s);$j++) {$new.=$s[$j].' ';}}
               else{for($j=0; $j<36;$j++) {$new.=$s[$j].' ';}}
               echo"$new</a></h3>";
                  echo"</div>";
                  echo"</div>";
                  echo"</div>";
 ...
 <script>
    function Myfiltercats(){
      ...
      var div2 = document.querySelectorAll(".bird");
    ...
    function Myfilterbirds(){
      ...
      var div3 = document.querySelectorAll(".bird");
      for (var d =0; d<div3.length; d++) {
        div3[d].style.display = "inline";
       }
    }
